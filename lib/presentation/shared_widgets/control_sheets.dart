      ...
      return Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            // ── Controles de reproducción (solo visible cuando hay audio) ──
            if (isPlaying) ...[
              const SizedBox(height: 16),
              _PlayerControls(ref: ref),
            ],
          ],
        ),
      );
...
/// Widget de controles de reproducción: barra de progreso, tiempo y seek.
class _PlayerControls extends ConsumerStatefulWidget {
  final WidgetRef ref;

  const _PlayerControls({required this.ref});
...
class _PlayerControlsState extends ConsumerState<_PlayerControls> {
  ...
  void initState() {
    super.initState();
    final repo = widget.ref.read(audioRepositoryProvider);

    repo.onDurationChanged.listen((d) {
      if (d != null && mounted) setState(() => _duration = d);
    });
    repo.onPositionChanged.listen((p) {
  ...
  Widget build(BuildContext context) {
    ...
    final currentValue = _isSliding ? _slidingValue : (durationSec > 0 ? positionSec / durationSec : 0.0);
    final isPaused = !repo.isPlaying;
      ...
      children: [
        ...
        SliderTheme(
          ...
          child: Slider(
            ...
            onChangeEnd: (v) {
              repo.seek(Duration(milliseconds: (v * durationSec * 1000).round()));
              setState(() {
                _isSliding = false;
              });
            },
